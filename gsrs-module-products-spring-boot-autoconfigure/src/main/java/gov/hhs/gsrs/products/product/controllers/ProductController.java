   ...
   // @GetGsrsRestApiMapping("/export/{etagId}/{format}")
    public ResponseEntity<Object> createExport(@PathVariable("etagId") String etagId,
        ...
        if (!etagObj.isPresent()) {
            ...
            Stream<Product> mstream = (Stream)(new EtagExportGenerator(entityManager, transactionManager, HttpRequestHolder.fromRequest(request))).generateExportFrom(this.getEntityService().getContext(), (ETag)etagObj.get()).get();
